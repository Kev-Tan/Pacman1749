...
void pacman_draw(Pacman* pman) {
	...
	*/
	...
	if (game_over) {
		...
		int moveData = pman->objData.moveCD;
		//printf("%d\n", moveData);
			switch(pman->objData.facing) //SWITCH PACMAN DIRECTION
			{
			case LEFT:
				if (moveData < 32) {
					al_draw_scaled_bitmap(pman->move_sprite, 48, 0,		//4th sprite 
						16, 16,
						drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
						draw_region, draw_region, 0
					);
				}
				else if (moveData >=32) {					//3rd sprite
			...
			case RIGHT:
				...
				else if (moveData >= 32) {			//1st sprite
					al_draw_scaled_bitmap(pman->move_sprite, 0, 0,
						16, 16,
						drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
						draw_region, draw_region, 0
			...
			case DOWN:
				...
				else if (moveData >= 32) {				//7th sprite
